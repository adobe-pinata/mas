...
3. Determines which domain mental model(s) should be re-synced based on what changed and prints the self-improve command (does not run it automatically).
...
The `mental-model/` subdirectory contains domain mental model command sets. Each domain has four commands that form a composable pattern: `plan`, `question`, `self-improve`, and `plan_build_improve`.

---

### ADW Mental Models (`/mental-model:adw:*`)

#### /mental-model:adw:plan
**Purpose:** Create implementation plans for Developer Workflow features, informed by ADW domain expertise.
**Usage:** `/mental-model:adw:plan "[user_request]"`
**Key behavior:**
1. Reads `.claude/commands/mental-model/adw/expertise.yaml` (the ADW mental model) and relevant codebase files.
...
**Usage:** `/mental-model:adw:question "[question]"`
**Key behavior:**
...
#### /mental-model:adw:plan_build_improve
**Purpose:** End-to-end ADW implementation — plan, build, then sync expertise in a single automated workflow.
**Usage:** `/mental-model:adw:plan_build_improve "[adw_implementation_request]" [human_in_the_loop]`
**Key behavior:**
1. Step 1: Spawns a subagent to run `/mental-model:adw:plan` — retrieves plan file path.
2. Step 2: Spawns a subagent to run `/implement` against that plan — retrieves build report.
3. Step 3: Spawns a subagent to run `/mental-model:adw:self-improve true` — retrieves self-improve report.
4. Compiles and returns a final workflow summary from all three steps.

---

### Experience Server Mental Models (`/mental-model:experience-server:*`)

#### /mental-model:experience-server:plan
...
#### /mental-model:experience-server:self-improve
**Purpose:** Validate and update the experience-server expertise file against the actual server codebase.
**Usage:** `/mental-model:experience-server:self-improve [true|false] [focus_area]`
**Key behavior:**
...
#### /mental-model:experience-server:plan_build_improve
**Purpose:** End-to-end experience-server implementation — plan, build, sync expertise.
**Usage:** `/mental-model:experience-server:plan_build_improve "[implementation_request]" [human_in_the_loop]`
**Key behavior:** Same three-step chain as the ADW variant — plan → implement → self-improve — using the experience-server mental model at each step.

---

### Experience Frontend Mental Models (`/mental-model:experience-frontend:*`)

#### /mental-model:experience-frontend:plan
...
**Usage:** `/mental-model:experience-frontend:plan "[user_request]" [prior_spec_path]`
**Key behavior:**
...
#### /mental-model:experience-frontend:question
...
**Usage:** `/mental-model:experience-frontend:question "[question]"`
**Key behavior:**
...
#### /mental-model:experience-frontend:self-improve
**Purpose:** Validate and update the frontend expertise file against the actual client codebase.
**Usage:** `/mental-model:experience-frontend:self-improve [true|false] [focus_area]`
**Key behavior:**
...
#### /mental-model:experience-frontend:plan_build_improve
**Purpose:** End-to-end frontend implementation — plan, build, sync expertise.
**Usage:** `/mental-model:experience-frontend:plan_build_improve "[implementation_request]" [human_in_the_loop]`
**Key behavior:** Same three-step chain — plan → implement → self-improve — using the frontend mental model at each step.

---

### Experience Integrations Mental Models (`/mental-model:experience-integrations:*`)

#### /mental-model:experience-integrations:plan
...
**Usage:** `/mental-model:experience-integrations:plan "[user_request]" [prior_spec_path]`
**Key behavior:**
...
#### /mental-model:experience-integrations:question
...
**Usage:** `/mental-model:experience-integrations:question "[question]"`
**Key behavior:**
...
#### /mental-model:experience-integrations:self-improve
**Purpose:** Validate and update the QA integrations expertise file against the actual codebase.
**Usage:** `/mental-model:experience-integrations:self-improve [true|false] [focus_area]`
**Key behavior:**
...
#### /mental-model:experience-integrations:plan_build_improve
**Purpose:** End-to-end QA integrations implementation — plan, build, sync expertise.
**Usage:** `/mental-model:experience-integrations:plan_build_improve "[implementation_request]" [human_in_the_loop]`
**Key behavior:** Same three-step chain — plan → implement → self-improve — using the experience-integrations mental model at each step.
...
#### /build-mental-models
**Purpose:** Generate all three QA domain mental model sets (experience-server, experience-frontend, experience-integrations) from the codebase in one orchestrated run.
**Usage:** `/build-mental-models`
**Key behavior:**
1. Step 1 (sequential): Spawns a Task agent to generate the experience-server mental model (5 files) — must complete first as it defines the API contracts.
2. Step 2 (parallel): Spawns two simultaneous Task agents — one for experience-frontend, one for experience-integrations — both reading experience-server's expertise.yaml as upstream context.
3. Step 3: Validates all 15 files exist, confirms YAML syntax valid for all three, checks line counts (all must be ≤ 1000).
4. Step 4: Commits all three mental model directories with `git commit`.
