...
"""
ADW Document Iso - Developer Workflow for documentation generation in isolated worktrees
...
def generate_documentation(
    ...
    # Documentation files are typically in app_docs/ directory
    path_pattern = r'app_docs/[a-z0-9\-]+\.md'
    match = re.search(path_pattern, raw_output)

    if match:
        doc_file_path = match.group(0)
        logger.info(f"Extracted documentation path: {doc_file_path}")
    else:
        # Fallback: look for any line ending with .md in app_docs
        for line in raw_output.split('\n'):
            line = line.strip()
            if 'app_docs' in line and line.endswith('.md'):
                doc_file_path = line
                logger.info(f"Extracted documentation path from line: {doc_file_path}")
                break
        else:
            # Last resort: use the whole response
            doc_file_path = raw_output
            logger.debug(f"Using raw output as doc path: {doc_file_path}")
...
def main():
    ...
    if state:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(adw_id, "ops", "Starting documentation"),
        )
    else:
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(adw_id, "ops", f"Documentation complete ✓\n\n{state.format_for_github()}"),
    )
