...
def check_env_vars() -> CheckResult:
    ...
    optional_vars = {
        "ANTHROPIC_API_KEY": "(Optional) Anthropic API Key for Claude Code - only needed for testing Claude CLI functionality",
        ...
        "AIO_STATE_NAMESPACE": "(Optional) AIO namespace for aio-lib-files screenshot uploads",
        "AIO_STATE_API_KEY": "(Optional) AIO API key for aio-lib-files screenshot uploads",
    }
...
def main():
    ...
    if not result.success:
        ...
        if any("GitHub CLI" in e for e in result.errors):
            print(f"   {step_num}. Install GitHub CLI: brew install gh")
            step_num += 1
