...
class R2Uploader:
    ...
    def _initialize(self) -> None:
        ...
        # Strip protocol prefix if present (defensive fix for user configuration)
        if self.public_domain.startswith("https://"):
            self.public_domain = self.public_domain[8:]
            self.logger.debug(f"Stripped https:// prefix from public domain: {self.public_domain}")
        elif self.public_domain.startswith("http://"):
            self.public_domain = self.public_domain[7:]
            self.logger.debug(f"Stripped http:// prefix from public domain: {self.public_domain}")
