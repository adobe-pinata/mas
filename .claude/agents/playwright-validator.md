...
1. **Initialize validation session:**
   - Create a timestamped directory for this validation run: `./.reports/e2e/playwright/YYYY-MM-DD_HH-MM-SS/`
...
5. **Per-route deep checks (run on every route validated):**
   - **Interactive element inventory:** Use `mcp__playwright__browser_evaluate` to list all focusable/interactive elements:
     ```js
     [...document.querySelectorAll('button,input,select,textarea,a,[tabindex]')]
       .map(el => ({ tag: el.tagName, text: el.textContent?.trim().slice(0,40), disabled: el.disabled, role: el.role }))
     ```
     Report which elements are enabled vs disabled — this catches UI state issues (e.g. Send button disabled until input).
   - **Error alert detection:** Check for ARIA alert elements and their exact text:
     ```js
     [...document.querySelectorAll('[role="alert"]')].map(el => el.textContent?.trim())
     ```
   - **Network request counts:** Inject a performance observer or read `window.performance.getEntriesByType('resource')` to count how many times each API endpoint was called — report exact counts (e.g. `/api/chat` x2), not just "repeated".
   - **Console error capture:** Use `mcp__playwright__browser_console_messages` to capture browser console output and quote exact error messages per route.
...
## Steps Executed
1. PASS  Navigate to [URL] — Screenshot: 01-initial-page.png
2. PASS  [Action description] — Screenshot: 02-[description].png
...
| Fatal JS crash / error boundary triggered | YES/NO |
| Visible inline error alerts (`role="alert"`) | YES/NO — [exact text if YES] |
| Interactive elements | [list: element, enabled/disabled state] |
| API calls fired (with counts) | [endpoint x N, endpoint x N] |
| Console errors | [exact messages or "none"] |
| Render status | PASS / PARTIAL PASS / FAIL |

**What is visible:** [description]

## Summary Table

| Route | Renders | No Crash | Error Alerts | API Call Counts | Verdict |
|---|---|---|---|---|---|
| `/` | PASS/FAIL | PASS/FAIL | [text or none] | [endpoint xN] | PASS/PARTIAL/FAIL |
...
## Issues Encountered
[Any errors, warnings, or unexpected behaviors — include exact console messages]
